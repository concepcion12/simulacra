...
try:
    ...
    SocketIO = None
    print(
        "Warning: Flask and flask-socketio not installed. Install with: "
...
class VisualizationServer:
    ...
    def __init__(self, data_streamer: DataStreamer, port: int = 5000, debug: bool = False):
        ...
        if Flask is None:
            raise ImportError(
                "Flask is required for visualization server. Install with: "
                "pip install flask flask-socketio"
            )
        ...
        def handle_interval_change(data):
            ...
            try:
                ...
                self.update_interval = max(
                    0.1,
                    min(10.0, new_interval)
                )  # Clamp between 0.1-10 seconds
    ...
    def start_server(self, threaded: bool = True) -> None:
        ...
        print(
            f"Starting visualization server on http://localhost:{self.port}"
        )
        ...
        if threaded:
            ...
            server_thread.start()
            print(
                f"Visualization server running at http://localhost:{self.port}"
            )
        else:
