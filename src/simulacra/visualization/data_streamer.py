...
from simulacra.utils.types import AgentID, Coordinate
...
class DataStreamer:
        ...
        for district in self.city.districts:
            ...
            for plot in district.plots:
                plots.append({
                    ...
                    'plot_type': plot.plot_type.name if hasattr(plot.plot_type, 'name') else str(plot.plot_type),
        ...
        for agent in self.simulation.agents:
            ...
            agent_info = {
                ...
                'state': {
                    ...
                    'addiction_level': agent.addiction_states.get('alcohol', type('obj', (object,), {'stock': 0})).stock
                },
                'visual_properties': {
                ...
                if plot.building:
                    ...
                    if hasattr(plot.building, 'current_occupants'):
                        ...
                        occupancy_info['occupancy'] = sum(
                            1 for agent in self.simulation.agents
                            if (hasattr(agent, 'current_location') and 
                                agent.current_location == plot.location) or
                               (hasattr(agent, 'home') and agent.home == plot.building)
                        )
                    
                    if occupancy_info['capacity'] > 0:
                        occupancy_info['occupancy_rate'] = occupancy_info['occupancy'] / occupancy_info['capacity']
    ...
    def _get_heat_map_data(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for location_key, agents in location_groups.items():
            ...
            if agents:
                avg_stress = sum(agent.internal_state.stress for agent in agents) / len(agents)
                avg_addiction = sum(
                    agent.addiction_states.get('alcohol', type('obj', (object,), {'stock': 0})).stock 
                    for agent in agents
                ) / len(agents)
                avg_wealth = sum(agent.internal_state.wealth for agent in agents) / len(agents)
